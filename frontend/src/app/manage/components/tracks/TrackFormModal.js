...
export default function TrackFormModal({ isOpen, onClose, onSubmit, editingTrack }) {
  const { t, lang } = useTranslation();
  const [form, setForm] = useState({ name: "", lanes_count: 2, length_meters: "" });
  ...
  useEffect(() => {
    if (editingTrack) {
      setForm({
        ...
        lanes_count: editingTrack.lanes_count || 2,
        length_meters: editingTrack.length_meters || "",
      });
    } else {
      setForm({ name: "", lanes_count: 2, length_meters: "" });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("lanes_count", parseInt(form.lanes_count));
      if (form.length_meters !== "") {
        formData.append("length_meters", parseFloat(form.length_meters));
      } else {
        formData.append("length_meters", "");
      }
  ...
  const hasImage = previewUrl || (editingTrack?.image_url && !imageDeleted);
      ...
      <div className="relative w-full max-w-md glass-panel p-6 rounded-2xl border border-white/10 bg-slate-900/90 shadow-2xl z-10 space-y-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-white/5 pb-3">
          <h2 className="text-lg font-bold text-slate-200">
            {editingTrack ? t.track.editTrack : t.track.registerTrack}
          </h2>
          <button 
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t.track.trackName}</label>
            <input
              type="text"
              required
              placeholder="Ex: Silverstone, Garage GP"
              value={form.name}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t.track.lanesLabel}</label>
              <input
                type="number"
                min="1"
                max="8"
                required
                value={form.lanes_count}
                onChange={(e) => setForm({ ...form, lanes_count: e.target.value })}
                className="w-full px-3 py-2.5 rounded-xl bg-slate-955 border border-white/5 text-slate-200 text-sm focus:outline-none focus:border-cyan-500/50 transition-colors"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t.track.lengthLabel}</label>
              <input
                type="number"
                step="0.01"
                placeholder="Ex: 12.5"
          ...
          <div className="space-y-3">
            <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">
              {editingTrack ? t.track.photoLabelKeep : t.track.photoLabelNew}
            </label>
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  {(previewUrl || editingTrack?.image_size_formatted) && (
                    <span className="text-xs font-mono-telemetry font-bold text-slate-400 bg-slate-955/60 border border-white/5 px-2 py-0.5 rounded-lg">
                      {previewUrl ? `${(selectedFile.size / 1024).toFixed(0)} KB` : editingTrack.image_size_formatted}
                    </span>
                  )}
                </div>
          ...
          <div className="flex gap-3 pt-3">
            ...
            >
              {submitting 
                ? (editingTrack ? t.track.saving : t.track.registering) 
                : (editingTrack ? t.track.save : t.track.register)}
            </button>
          </div>
        </form>
      </div>
